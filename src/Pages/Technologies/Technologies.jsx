...
export default function Technologies() {
    const Techs = [
        ...
        },
        {
            ...
            "image" : "https://io.google/2022/app/images/web-logo.svg"
        }
    ...
    return (
        <div className="flex flex-col items-center justify-center text-center">
          ...
          {Techs.reduce((rows, key, index) => (index % 5 == 0 ? rows.push([key]) : rows[rows.length-1].push(key)) && rows, []).map((row, rowIndex) => (
            <div className="flex justify-center mb-10" key={rowIndex}>
              {row.map((item, index) => (
                <a
                  className="text-inherit hover:text-primary transition-all mx-6"
                  key={index}
                  href={item.link}
                  target="_blank"
                  rel="noreferrer"
